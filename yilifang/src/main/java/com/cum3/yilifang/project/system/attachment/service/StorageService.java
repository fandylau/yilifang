package com.cum3.yilifang.project.system.attachment.service;
...
import com.cum3.yilifang.project.system.attachment.utils.FileMD5Util;

/**
 * 描述
 * 
 * @author Fandy Liu
 * @created 2018年9月7日 上午11:07:15
 */
...
public class StorageService {
    ...
    @Autowired
    public StorageService(@Value("${breakpoint.upload.dir}") String location) {
    ...
    /*public void uploadFileRandomAccessFile(MultipartFileParam param) throws IOException {
        ...
        if (isOk) {
            ...
            logger.debug("upload complete !! {} name={}",flag,fileName);
        }
    }*/
    ...
    public void uploadFileByMappedByteBuffer(MultipartFileParam param) throws IOException {
        ...
        if (isOk) {
            boolean flag = renameFile(tmpFile, fileName);
            logger.info("upload complete !!{} name={}",flag,fileName);
        }
    }
    ...
    private boolean checkAndSetUploadProgress(MultipartFileParam param, String uploadDirPath) throws IOException {
        ...
        // 把该分段标记为 true 表示完成
        logger.debug("set part {} complete",param.getChunk());
        ...
        for (int i = 0; i < completeList.length && isComplete == Byte.MAX_VALUE; i++) {
            ...
            logger.debug("check part {} complete?:{}",i ,completeList[i]);
        }
