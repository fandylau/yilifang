...
public class ScheduleUtils {
     ...
     * @param job
     */
    public static void startDelayTimeJob(Scheduler scheduler,Job job) {
        try {
            long fTime = System.currentTimeMillis() + job.getDelayMillisecond();
            // 构建job信息
            JobDetail jobDetail = JobBuilder.newJob(ScheduleJob.class).withIdentity(getJobKey(job.getJobId())).build();
            Trigger trigger = TriggerBuilder.newTrigger().withIdentity(getTriggerKey(job.getJobId())).startAt(new Date(fTime))
     ...
     * 
     * 创建定时器 按规定时间点执行
     * @created 2018年10月29日 下午7:30:06
     * @param scheduler
     * @param job
     */
    public static void startFixedDateJob(Scheduler scheduler,Job job) {
        try {
            // 构建job信息
            JobDetail jobDetail = JobBuilder.newJob(ScheduleJob.class).withIdentity(getJobKey(job.getJobId())).build();
            Trigger trigger = TriggerBuilder.newTrigger().withIdentity(getTriggerKey(job.getJobId())).startAt(job.getTriggerStartTime())
                    .withSchedule(SimpleScheduleBuilder.simpleSchedule().withRepeatCount(0)).build();
            // 放入参数，运行时的方法可以获取
            jobDetail.getJobDataMap().put(ScheduleConstants.TASK_PROPERTIES, job);
            scheduler.scheduleJob(jobDetail, trigger);
    ...
    public static CronScheduleBuilder handleCronScheduleMisfirePolicy(Job job, CronScheduleBuilder cb) {
        switch (job.getMisfirePolicy()) {
            ...
            default:
                throw ExceptionUtils.exp("The task misfire policy '" + job.getMisfirePolicy()
